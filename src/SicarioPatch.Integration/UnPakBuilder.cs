...
[PublicAPI]
public sealed class UnPakBuilder : IModBuilder
{
    ...
    public Task<(bool Success, FileSystemInfo Output)> RunBuildAsync(IBuildContext buildContext, string targetFileName)
    {
        ...
        var targetBuildPath = Path.IsPathRooted(targetFileName)
            ? targetFileName
            : Path.Combine(ctx.WorkingDirectory.FullName, targetFileName);
