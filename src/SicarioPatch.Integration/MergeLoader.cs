...
/// </summary>
[PublicAPI]
public sealed class MergeLoader
{
    ...
    private IEnumerable<FileInfo> GetAllMods()
    {
        ...
        return allPaks.Where(static p => Path.GetFileNameWithoutExtension(p) != "ProjectWingman-WindowsNoEditor")
            ...
            .OrderBy(f => Path.GetRelativePath(pakPath, f.FullName));
    }

    private static Record? GetPreset(PakFile? file)
    {
        return file?.Records.FirstOrDefault(r =>
            r.GetVirtualPath(file).Contains("sicario") && Path.GetExtension(r.GetVirtualPath(file)) == ".dtp");
    }

    private static Record? GetRequest(PakFile? file)
    {
        return file?.Records.FirstOrDefault(r =>
            r.GetVirtualPath(file).Contains("_meta/sicario") && Path.GetExtension(r.GetVirtualPath(file)) == ".json");
    }
    ...
    public Dictionary<string, WingmanPreset> LoadPresetsFromMods()
    {
        ...
        foreach (var pakFileInfo in allPaks)
            try
            {
                ...
                if (embed?.Mods != null && embed.Mods.Any()) builtMods.Add(pakFileInfo.FullName, embed);
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
    ...
    public Dictionary<string, EmbeddedResources> GetEmbeddedAssets()
    {
        ...
        foreach (var pakFileInfo in allPaks)
            ...
            {
                Console.WriteLine(
                    $"Error encountered loading presets from {pakFileInfo.Name}. You may need to rebuild/re-download this mod: {e.GetType()}");
                // Console.WriteLine(e);
                //ignored
            }
...
[PublicAPI]
public sealed record EmbeddedResources
{
