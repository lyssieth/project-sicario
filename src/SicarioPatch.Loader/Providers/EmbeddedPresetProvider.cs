...
public sealed class EmbeddedResourceProvider : IMergeProvider<WingmanMod>
{
    ...
    public EmbeddedResourceProvider(MergeLoader mergeLoader, IEngineInfoProvider engineInfo,
    ...
    public IEnumerable<MergeComponent<WingmanMod>> GetMergeComponents(List<string>? searchPaths)
    {
        ...
        var embeddedInputs = embeddedPresets
            .Select(static m => m.Value.ModParameters)
        ...
        components.Add(new MergeComponent<WingmanMod>
        {
            ...
            Mods = embeddedPresets.SelectMany(static ep => ep.Value.Mods),
            MergedResources = embeddedPresets.ToDictionary(static em => em.Key,
                static v => string.Join(";", v.Value.Mods.Select(static m => m.GetLabel()))),
            Parameters = embeddedInputs,
            Priority = 1
        });
        var mergedInputs = requests
            .Select(static m => m.Value.TemplateInputs)
            .Aggregate(new Dictionary<string, string>(), static (total, next) => next.MergeLeft(total));
        components.Add(new MergeComponent<WingmanMod>
        {
            Name = "sicarioRequests",
            Mods = requests.SelectMany(static m => m.Value.Mods),
            Parameters = mergedInputs,
            Priority = 3,
            Message = $"[dodgerblue2]Loaded [bold]{requests.Count}[/] Sicario mods for rebuild[/]",
            MergedResources = requests.ToDictionary(static em => em.Key,
                static v => string.Join(";", v.Value.Mods.Select(static m => m.GetLabel())))
        });
        return components;
    }
